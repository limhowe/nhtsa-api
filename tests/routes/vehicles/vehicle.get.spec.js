...
describe(`GET ${ENDPOINT}/:modelYear/:manufacturer/:model`, () => {
  it('#200 - should return correct results for valid params', done => {
    const {input : {modelYear, manufacturer, model}, response : {Count: expectCount, Results: expectResults}} = correctCase;

    server.get(`${ENDPOINT}/${modelYear}/${manufacturer}/${model}`)
      .end((err, res) => {
  ...
  it('#200 - should return empty results', done => {
    const {input : {modelYear, manufacturer, model}, response : {Count: expectCount, Results: expectResults}} = emptyCase;
